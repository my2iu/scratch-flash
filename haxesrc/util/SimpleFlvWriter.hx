...
class SimpleFlvWriter
{
    private static var _instance : SimpleFlvWriter;
    ...
    public function createFile(bytes : ByteArray, pWidth : Int, pHeight : Int, pFramesPerSecond : Float, pDurationInSeconds : Float = 0) : Void{
        /*
			Parameters:
    ...
    private function videoData() : ByteArray{
        ...
        writeUI4_12(v, as3hx.Compat.parseInt(blockWidth / 16) - 1, frameWidth);
        // blockheight/16-1 (4bits) + imageheight (12bits)
        writeUI4_12(v, as3hx.Compat.parseInt(blockHeight / 16) - 1, frameHeight);
        
        // VIDEODATA > SCREENVIDEOPACKET > IMAGEBLOCKS:
        
        var yMax : Int = as3hx.Compat.parseInt(frameHeight / blockHeight);
        var yRemainder : Int = frameHeight % blockHeight;
        if (yRemainder > 0)             yMax += 1;
        
        var xMax : Int = as3hx.Compat.parseInt(frameWidth / blockWidth);
    ...
    private function metaData() : ByteArray
    {
        ...
        b.writeByte(1);
        b.writeByte(as3hx.Compat.parseInt(true));
    ...
    private function writeUI24(stream : Dynamic, p : Int) : Void{
        var byte1 : Int = p >> 16;
    ...
    private function writeUI16(stream : Dynamic, p : Int) : Void{
        stream.writeByte(p >> 8);
        stream.writeByte(p & 0xff);
    }
    
    private function writeUI4_12(stream : Dynamic, p1 : Int, p2 : Int) : Void{
