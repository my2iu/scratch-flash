...
class Renderer
{
    ...
    public static function renderToSprite(spr : Sprite, rootSVG : SVGElement) : Void{
        // Populate the given sprite with DisplayObjects (e.g. SVGBitmap) for the subelements of rootSVG.
        if (rootSVG == null)             return;
        for (el/* AS3HX WARNING could not determine type for var: el exp: EField(EIdent(rootSVG),subElements) type: null */ in rootSVG.subElements){
            appendElementToSprite(el, spr);
        }
    }
    
    private static function appendElementToSprite(el : SVGElement, spr : Sprite) : Void{
