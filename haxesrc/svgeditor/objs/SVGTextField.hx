...
class SVGTextField extends TextField implements ISVGEditable
{
    ...
    public function redraw(forHitTest : Bool = false) : Void{
        var fixup : Bool = (type == TextFieldType.INPUT && element.text.length < 4);
    ...
    public function clone() : ISVGEditable{
        ...
        copy.redraw();
        return try cast(copy, ISVGEditable) catch(e:Dynamic) null;
    }
}
