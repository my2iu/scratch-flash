...
class DashDrawer
{
    ...
    public static function drawPoly(g : Graphics, points : Array<Dynamic>, dashLength : Int, dashColor : Int) : Void{
        // Get the dash bitmap
        var bd : BitmapData = getDashBitmap(dashLength, dashColor);
    ...
    public static function getDashBitmap(dashLength : Int, dashColor : Int) : BitmapData{
        ...
        var bd : BitmapData = Reflect.field(lineBitmaps, key);
        if (bd == null) {
            bd = Reflect.setField(lineBitmaps, key, generateDashBitmap(dashLength, dashColor));
        }
