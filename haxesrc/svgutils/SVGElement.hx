...
class SVGElement {
	...
	public var subElements:Array<Dynamic>;
	...
	public function allElements():Array<Dynamic> {
	...
	public function isBackDropBG():Bool {
		return (attributes.indexOf('scratch-type') != -1 && attributes['scratch-type'].indexOf('backdrop-') == 0);
	}

	public function isBackDropFill():Bool {
		return (attributes.indexOf('scratch-type') != -1 && attributes['scratch-type'] == 'backdrop-fill');
	}
	...
	public function getAttribute(key:String, defaultIfMissing:Dynamic = undefined):Dynamic {
		...
		if(attributes.hasOwnProperty(key)) {
			...
			else if(Std.is(defaultIfMissing, Number) && rawValue == 'undefined')
	...
	public function setAttribute(key:String, value:Dynamic):Void {
		if (value == null || value == undefined) {
			attributes[key] = null; // delete attributes[key];
		} else {
			attributes[key] = value;
		}
	}

	public function deleteAttributes(keys:Array<Dynamic>):Void {
		for (k in keys) attributes[k] = null; //delete attributes[k];
	}

	public function extractNumericArgs(input:String):Array<Dynamic> {
		...
		for (s in numStrings) result.push(Number(s));
		return result;
	}
	...
	private static inline var namedColors:Dynamic = {
		'aliceblue': 0xF0F8FF,
