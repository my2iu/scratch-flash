...
class SoundBank
{
    ...
    public static var instrumentNames : Array<Dynamic> = [
        "Piano", 
        "Electric Piano", 
        "Organ", 
        "Guitar", 
    ...
    public static var drumNames : Array<Dynamic> = [
        "Snare Drum", 
        "Bass Drum", 
        "Side Stick", 
        "Crash Cymbal", 
    ...
    public static function getNotePlayer(instNum : Int, midiKey : Int) : NotePlayer{
        ...
        Instr.initSamples();
        var r : Array<Dynamic> = getNoteRecord(instNum, midiKey);
        var env : Array<Dynamic> = ((r.length > 5)) ? r[5] : null;
        return new NotePlayer(Instr.samples[r[1]], pitchForKey(r[2]), r[3], r[4], env);
    }
    
    private static function getNoteRecord(instNum : Int, midiKey : Int) : Array<Dynamic>{
        // Get a note record for the given instrument number.
        if ((instNum < 0) || (instNum >= instruments.length))             instNum = 0;
        var keyRanges : Array<Dynamic> = instruments[instNum];
        for (r in keyRanges){
            var topOfKeyRange : Int = r[0];
            if (midiKey <= topOfKeyRange)                 return r;
        }
        return keyRanges[keyRanges.length - 1];
    }
    
    private static function pitchForKey(midiKey : Float) : Float{
        return 440 * Math.pow(2, (midiKey - 69) / 12);
    }
    
    public static function getDrumPlayer(drumNum : Int, isMIDI : Bool, secs : Float) : NotePlayer{
        // Return a NotePlayer for the given drum number. If isMIDI is true,
        // use a Scratch 1.4 MIDI drum number (35..81).
        Instr.initSamples();
        var entry : Array<Dynamic> = (isMIDI) ? midiDrums[drumNum - 35] : drums[drumNum - 1];
        if (entry == null)             entry = drums[2];
        var loopStart : Int = -1;
        var loopEnd : Int = -1;
        var env : Array<Dynamic> = null;
        if (entry.length >= 4) {
            loopStart = entry[2];
            loopEnd = entry[3];
        }
    ...
    public static function checkInstruments() : Void{
        Instr.initSamples();
        var lastOne : Array<Dynamic>;
        for (instr/* AS3HX WARNING could not determine type for var: instr exp: EIdent(instruments) type: null */ in instruments){
            for (smpl/* AS3HX WARNING could not determine type for var: smpl exp: EIdent(instr) type: null */ in instr){
                if (smpl == lastOne)                     continue;
                var sampleName : String = smpl[1];
                var key : Int = smpl[2];
                var buf : ByteArray = Instr.samples[sampleName];
                if (buf == null)                     trace(" ***** missing: " + sampleName);
                ...
                if (loopStart >= 0) {
                    ...
                    var keyPitch : Float = pitchForKey(key);
                    var oneCycle : Float = 22050 / keyPitch;
                    var cycles : Int = Math.round(loopLen / oneCycle);
                    var loopPitch : Float = 22050 / (loopLen / cycles);
                    var cents : Float = 1200 * Math.log(keyPitch / loopPitch) / Math.LN2;
    ...
    private static var instruments : Array<Dynamic> = [
    ...
    private static var drums : Array<Dynamic> = [
        ["SnareDrum", 0], 
        ["Tom", 0], 
        ["SideStick", 0], 
        ["Crash", -7], 
    ...
    // Old 2.0 drums
    private static var oldDrums : Array<Dynamic> = [
        ["BassDrum", -7], 
        ["SideStick", 0], 
        ["SnareDrum", 0], 
        ["Tom", -5, 7260, 7483, 3.2], 
    ...
    private static var midiDrums : Array<Dynamic> = [
        ["BassDrum", -4],   //35  
        ["BassDrum", 0], 
        ["SideStick", 0], 
        ["SnareDrum", 0], 
