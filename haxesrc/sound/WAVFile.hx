...
class WAVFile
{
    ...
    public static function encode(sampleData : ByteArray, sampleCount : Int, rate : Int, doCompress : Bool) : ByteArray{
    ...
    public static function decode(waveData : ByteArray) : Dynamic{
        ...
        if (formatChunk.length < 16)             throw cast(("WAVFile: format chunk is too small"), Error);
        var encoding : Int = formatChunk.readUnsignedShort();
        ...
        if (formatChunk.length >= 18 && encoding == 0xFFFE) {
            var extensionSize : Int = formatChunk.readUnsignedShort();
            if (extensionSize == 22) {
        ...
        if (encoding == 1) {
            if (!((result.bitsPerSample == 8) || (result.bitsPerSample == 16))) {
                throw cast(("WAVFile: can only handle 8-bit or 16-bit uncompressed PCM data"), Error);
            }
            ...
            if (formatChunk.length < 20)                 throw cast(("WAVFile: adpcm format chunk is too small"), Error);
            if (result.channels != 1)                 throw cast(("WAVFile: adpcm supports only one channel (monophonic)"), Error);
            ...
            throw cast(("WAVFile: unknown encoding " + encoding), Error);
        }
        
        return result;
    }
    
    public static function extractSamples(waveData : ByteArray) : Array<Int>{
        var result : Array<Int> = new Array<Int>();
        var info : Dynamic = WAVFile.decode(waveData);
        var i : Int;
        var v : Int;
        if (info.encoding == 1) {
            waveData.position = info.sampleDataStart;
            for (i in 0...info.sampleCount){
            ...
            waveData.position = info.sampleDataStart;
            for (i in 0...info.sampleCount){
                var f : Float = (info.bitsPerSample == (32) ? waveData.readFloat() : waveData.readDouble());
                if (f > 1.0)                     f = 1.0;
                if (f < -1.0)                     f = -1.0;
                v = f * 0x7fff;
                result.push(v);
    ...
    private static function dataChunkStartAndSize(waveData : ByteArray) : Array<Dynamic>{
    ...
    private static var stepTable : Array<Dynamic> = [
    ...
    private static var indexTable : Array<Dynamic> = [
        -1, -1, -1, -1, 2, 4, 6, 8, 
        -1, -1, -1, -1, 2, 4, 6, 8];
    ...
    private static function imaDecompress(compressedData : ByteArray, blockSize : Int) : ByteArray{
        ...
        if (compressedData == null)             return out;
