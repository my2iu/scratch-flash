...
class ScratchSoundPlayer
{
    
    public static var activeSounds : Array<Dynamic> = [];
    
    public static function stopAllSounds() : Void{
        // Stop playing all currently playing sounds
        var oldSounds : Array<Dynamic> = activeSounds;
        activeSounds = [];
        for (player in oldSounds)player.stopPlaying();
    }
    ...
    private var bytePosition : Int;  // use our own position to allow sound data to be shared  
    private var soundChannel : SoundChannel;
    private var lastBufferTime : Int;
    
    // volume support
    public var client : Dynamic;
    private var volume : Float = 1.0;
    private var lastClientVolume : Float;
    
    // interpolation function and state
    private var getSample : Function;
    private var fraction : Float = 0.0;
    private var thisSample : Int;private var nextSample : Int;
    
    public function new(wavFileData : ByteArray)
    {
        getSample = getSample16Uncompressed;
        if (wavFileData != null) {
            try{
                var info : Dynamic = WAVFile.decode(wavFileData);
    ...
    public function isPlaying(snd : ByteArray = null) : Bool{
        return (Lambda.indexOf(activeSounds, this) > -1 && (!snd || soundData == snd));
    }
    
    public function atEnd() : Bool{return soundChannel == null;
    }
    ...
    public function stopPlaying() : Void{
        ...
        var i : Int = Lambda.indexOf(activeSounds, this);
    ...
    private function stopIfAlreadyPlaying() : Void{
        if (scratchSound == null)             return;
        var stopped : Bool;
        var i : Int;
        for (i in 0...activeSounds.length){
        ...
        if (stopped) {
            var stillPlaying : Array<Dynamic> = [];
            for (i in 0...activeSounds.length){
    ...
    private function noteFinished() : Void{
    ...
    private function writeSampleData(evt : SampleDataEvent) : Void{
        var i : Int;
        if ((lastBufferTime != 0) && ((Math.round(haxe.Timer.stamp() * 1000) - lastBufferTime) > 230)) {
        ...
        for (i in 0...4096){
            var n : Float = interpolatedSample();
            data.writeFloat(n);
            data.writeFloat(n);
        }
        if ((bytePosition >= endOffset) && (lastBufferTime == 0)) {
            lastBufferTime = Math.round(haxe.Timer.stamp() * 1000);
        }
    }
    
    private function interpolatedSample() : Float{
        fraction += stepSize;
        while (fraction >= 1.0){
    ...
    private var indexTable : Array<Dynamic> = [-1, -1, -1, -1, 2, 4, 6, 8, -1, -1, -1, -1, 2, 4, 6, 8];
    private var stepTable : Array<Dynamic> = [
