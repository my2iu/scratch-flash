...
class NotePlayer extends ScratchSoundPlayer
{
    ...
    private var loopLength : Float;
    
    // Volume Envelope
    private var envelopeValue : Float = 1;
    private var samplesSinceStart : Int = 0;
    private var attackEnd : Int = 0;
    private var attackRate : Float = 0;
    private var holdEnd : Int = 0;
    private var decayRate : Float = 1;
    
    public function new(soundData : ByteArray, originalPitch : Float, loopStart : Int = -1, loopEnd : Int = -1, env : Array<Dynamic> = null)
    {
        ...
        getSample =function() : Int{return 0;
        ...
        if ((loopStart >= 0) && (loopStart < endOffset)) {
            ...
            var oneCycle : Float = 22050 / originalPitch;
    ...
    public function setNoteAndDuration(midiKey : Float, secs : Float) : Void{
        midiKey = Math.max(0, Math.min(midiKey, 127));
        var pitch : Float = 440 * Math.pow(2, (midiKey - 69) / 12);  // midi key 69 is A (440 Hz)  
        stepSize = pitch / (2 * originalPitch);  // adjust for original sampling rate of 22050  
        setDuration(secs);
    }
    
    public function setDuration(secs : Float) : Void{
        samplesSinceStart = 0;
    ...
    override private function interpolatedSample() : Float{
        if (samplesRemaining-- <= 0) {noteFinished();return 0;
        }
        index += stepSize;
        if (index >= endOffset) {
            if (!isLooped)                 return 0;
            var sub : Float = loopLength - ((index - endOffset) % loopLength);
            if (sub == 0)                 index = endOffset - loopLength
            else index = endOffset - sub;
        }
        var i : Int = as3hx.Compat.parseInt(index);
        var frac : Float = index - i;
        var byteIndex : Int = i << 1;
        var result : Int = (soundData[byteIndex + 1] << 8) + soundData[byteIndex];
        var curr : Float = (result <= (32767) ? result : result - 65536);
        ...
        var sample : Float = (curr + (frac * (next - curr))) / 100000;  // xxx 32000; attenuate...  
