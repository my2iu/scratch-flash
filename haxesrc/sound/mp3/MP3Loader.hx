...
class MP3Loader
{
    ...
    public static function convertToScratchSound(sndName : String, sndData : ByteArray, whenDone : Function) : Void{
        ...
        var mp3Info : Dynamic = new MP3FileReader(sndData).getInfo();
    ...
    public static function extractSamples(sndName : String, mp3Snd : Sound, mp3SampleCount : Int, whenDone : Function) : Void{
        ...
        var extractedSamples : Array<Int> = new Array<Int>();
        ...
        function convertNextChunk() : Void{
            ...
            for (i in 0...count){
                ...
                var s : Float = buf.readFloat() + buf.readFloat();
    ...
    private static function generateSound(mp3Source : MP3FileReader, whenDone : Function) : Void{
        function swfCreated(evt : Event) : Void{
            var loaderInfo : LoaderInfo = try cast(evt.currentTarget, LoaderInfo) catch(e:Dynamic) null;
            var soundClass : Class<Dynamic> = Type.getClass(loaderInfo.applicationDomain.getDefinition("SoundClass"));
            whenDone(Type.createInstance(soundClass, []));
        };
    ...
    private static function appendBytes(swf : ByteArray, bytes : Array<Dynamic>) : Void{
        for (i in 0...bytes.length){swf.writeByte(bytes[i]);
        }
    }
    
    private static var soundClassSwfBytes1 : Array<Dynamic> = [0x46, 0x57, 0x53, 0x09];
