...
import svgeditor.tools.Bitmap;
import svgeditor.tools.BitmapData;
import svgeditor.tools.DrawProperties;
import svgeditor.tools.ImageEdit;
import svgeditor.tools.Point;
import svgeditor.tools.Rectangle;
...
@:final class BitmapPencilTool extends SVGTool
{
    
    private var eraseMode : Bool;  // true if this is the eraser tool  
    ...
    public function new(editor : ImageEdit, eraseMode : Bool = false)
    {
        super(editor);
    ...
    private function startStroke() : Void{
        ...
        var eraseWithWhite : Bool = editor.isScene && (eraseMode || (brushColor == 0));
    ...
    private function mouseMove(evt : MouseEvent) : Void{
        if (!editor.isActive())             return;
        moveFeedback();
        if (brush != null) {  // drawing or erasing  
            var p : Point = penPoint();
            if (lastPoint != null)                 drawLine(lastPoint.x, lastPoint.y, p.x, p.y)
            else drawAtPoint(p);
            lastPoint = p;
        }
    }
    ...
    private function mouseUp(evt : MouseEvent) : Void{
        if (brush != null)             editor.saveContent();
    ...
    private function updateFeedback() : Void{
        ...
        getPenProps();
        var outlineOnly : Bool = eraseMode || (brushColor == 0);
    ...
    private function setFeedbackVisibility() : Void{
        ...
        var inDrawingArea : Bool = r.containsPoint(new Point(editor.mouseX, editor.mouseY));
        if (inDrawingArea) {
            Mouse.hide();
    ...
    private function removeFeedback() : Void{
        if (feedbackObj != null && feedbackObj.parent)             feedbackObj.parent.removeChild(feedbackObj);
        feedbackObj = null;
    }
    ...
    private function drawAtPoint(p : Point) : Void{
        ...
        var isErasing : Bool = eraseMode || (brushColor == 0);
    ...
    private function makeBrush(diameter : Int, c : Int, bgColor : Int = 0, outlineOnly : Bool = false) : BitmapData{
    ...
    private function drawCircle(diameter : Int, c : Int, bm : BitmapData, outlineOnly : Bool) : Void{
